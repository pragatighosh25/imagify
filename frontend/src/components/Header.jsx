...
const Header = () => {
  ...
  return (
    ...
    >
      ...
      <motion.button className='sm:text-lg text-white bg-black w-auto mt-8 px-12 py-2.5 rounded-full items-center gap-2'
      ...
      className='flex flex-wrap justify-center mt-16 gap-3'>
        {Array(6).fill('').map((item, index) => (
          ...
          key={index} src={index%2===0 ? assets.sample_img_1 : assets.sample_img_2} alt='' width={70} className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10'  />
        ))}
      </motion.div>
      <motion.p
      initial={{opacity: 0}}
